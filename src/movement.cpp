...
sf::Sprite createCat(sf::Texture &texture, int fWidth, int fHeight,
...
void handleInput(sf::Sprite &cat, bool &isMoving, float &velocityX,
                 float &velocityY, bool &onGround, float jumpStrength,
                 sf::Clock &jumpClock) {
  isMoving = false;
  velocityX = 0.0f;
...
void updateAnimation(sf::Sprite &cat, sf::Texture &catTex, sf::Texture &jumpTex,
                     sf::IntRect &rect, sf::IntRect &jumpRect,
                     sf::Clock &jumpClock, sf::Clock &fallClock,
                     bool &startedFalling, bool isMoving, sf::Clock &animClock,
                     float animSpeed, int &frame, int total, int width, int pad,
                     bool &onGround, float velocityY) {
  if (onGround) {
