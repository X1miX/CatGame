...
sf::Sprite createObject(float x, float y, const sf::Texture &texture,
...
void handlePlatformCollisions(sf::Sprite &cat,
                              const std::vector<sf::Sprite> &platforms,
                              float dx, float &dy, bool &onGround) {
  cat.move(dx, 0);
...
void checkSpikeCollisions(sf::Sprite &cat,
                          const std::vector<sf::Sprite> &spikes,
                          sf::Vector2f spawnPoint, float &dy) {
