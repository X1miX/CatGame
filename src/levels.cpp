...
bool internalLevelLoop(sf::RenderWindow &win, sf::Vector2f spawnpoint, std::vector<sf::Sprite> &platforms, std::vector<sf::Sprite> &spikes, sf::Sprite &key, sf::Sprite &door, sf::Texture &catTex, sf::Texture &jumpTex, sf::Texture &bgTex) {
    sf::Sprite bg(bgTex);
    bg.setScale((float)win.getSize().x / bg.getLocalBounds().width, (float)win.getSize().y / bg.getLocalBounds().height);

    sf::Sprite cat = createCat(catTex, 180, 160, win.getSize().x, win.getSize().y);
    cat.setScale(0.5f, 0.5f); 
    cat.setPosition(spawnpoint);

    float vy = 0, vx = 0;
    bool onGround = false, isMoving = false, keyPicked = false, startedFalling = false;
    ...
    while (win.isOpen()) {
        ...
        if (!keyPicked && keyStarted) {
            if (keyTimer.getElapsedTime().asSeconds() > keySwitchTime) {
                keyRect.left = keyFrameIdx * static_cast<int>(keyFrameWidth);
                key.setTextureRect(keyRect);
                if (way) keyFrameIdx++; else keyFrameIdx--;
        ...
        updateAnimation(cat, catTex, jumpTex, catRect, jumpRect, jumpClock, fallClock, startedFalling, isMoving, animClock, 0.15f, frame, 4, 180, 20, onGround, vy);
...
bool runLevel1(sf::RenderWindow &win, sf::Texture &catTex, sf::Texture &jumpTex, sf::Texture &keyTex, sf::Texture &bgTex, sf::Texture &obj1Tex, sf::Texture &obj2Tex, sf::Texture &spikeTex, sf::Texture &doorTex) {
    ...
    sf::Sprite k = createObject(950, screenH - 550, keyTex, sf::IntRect(1100,0,100,200)); 
    k.setScale(0.3f, 0.3f);
    
    sf::Sprite d = createObject(1600, screenH - 215, doorTex, sf::IntRect(0,0,270,230)); 
...
bool runLevel2(sf::RenderWindow &win, sf::Texture &catTex, sf::Texture &jumpTex, sf::Texture &keyTex, sf::Texture &bgTex, sf::Texture &obj1Tex, sf::Texture &obj2Tex, sf::Texture &spikeTex, sf::Texture &doorTex) {
    ...
    sf::Sprite sp = createObject(800, screenH - 164, spikeTex, sf::IntRect(0,0,800,160)); // Шире в 2 раза
    sp.setScale(0.4f, 0.4f);
    s.push_back(sp);

    sf::Sprite k = createObject(100, screenH - 400, keyTex, sf::IntRect(1100,0,100,200)); 
    k.setScale(0.3f, 0.3f);
    
    sf::Sprite d = createObject(1500, screenH - 215, doorTex, sf::IntRect(0,0,270,230)); 
    d.setScale(0.5f, 0.5f);
...
bool runLevel3(sf::RenderWindow &win, sf::Texture &catTex, sf::Texture &jumpTex, sf::Texture &keyTex, sf::Texture &bgTex, sf::Texture &obj1Tex, sf::Texture &obj2Tex, sf::Texture &spikeTex, sf::Texture &doorTex) {
    ...
    sf::Sprite k = createObject(900, screenH - 650, keyTex, sf::IntRect(1100,0,100,200)); 
    k.setScale(0.3f, 0.3f);
    
    sf::Sprite d = createObject(1500, screenH - 415, doorTex, sf::IntRect(0,0,270,230)); 
    d.setScale(0.5f, 0.5f);

    return internalLevelLoop(win, {50, screenH - 300}, p, s, k, d, catTex, jumpTex, bgTex);
}